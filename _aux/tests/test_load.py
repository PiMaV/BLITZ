...
def test_apply_floor_abs_zeros_below_threshold():
    from blitz.data.load import apply_floor_abs

    arr = np.array([[0.0, 0.4, 1.0], [0.5, 2.0, -0.3]], dtype=np.float32)
    out = apply_floor_abs(arr, 0.5)
    expected = np.array([[0.0, 0.0, 1.0], [0.5, 2.0, 0.0]], dtype=np.float32)
    np.testing.assert_array_equal(out, expected)


def test_apply_floor_abs_default_off():
    from blitz.data.load import apply_floor_abs

    arr = np.array([0.1, 1.0], dtype=np.float32)
    np.testing.assert_array_equal(apply_floor_abs(arr, None), arr)
    np.testing.assert_array_equal(apply_floor_abs(arr, 0.0), arr)


def test_load_array_floor_abs(tmp_path):
    arr = np.array([[0.0, 0.4, 1.0], [0.5, 2.0, -0.3]], dtype=np.float32)
    path = tmp_path / "stack.npy"
    np.save(path, arr)

    with patch("blitz.data.load.settings") as mock_settings:
        mock_settings.get.return_value = 100000000
        loaded = DataLoader(floor_abs=0.5).load(path)
        defaulted = DataLoader().load(path)

    # DataLoader: (H, W) → (1, W, H, C) after swapaxes + ensure_4d
    got = np.squeeze(loaded.image)
    expected = np.array([[0.0, 0.5], [0.0, 2.0], [1.0, 0.0]], dtype=np.float32)
    np.testing.assert_allclose(got, expected)

    untouched = np.squeeze(defaulted.image)
    expected_off = np.array([[0.0, 0.5], [0.4, 2.0], [1.0, -0.3]], dtype=np.float32)
    np.testing.assert_allclose(untouched, expected_off)
