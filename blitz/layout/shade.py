...
class ShadeAdapter:
    ...
    def set_precached(self, on: bool) -> None:
        want = bool(on)
        ...
        if not self._precached:
            ...
            if self._preview:
                self._schedule()
            return
        self._azimuth = float(snap_azimuth_deg(self._azimuth))
        if not self._preview:
            self._preview = True
        self._rebuild_atlas()
        ...
        *,
        ...
        if self._precached:
            if azimuth is not None:
                self._azimuth = float(snap_azimuth_deg(azimuth))
    ...
    def _on_bin_ready(self, generation: int, az: int, shade: object) -> None:
        ...
        if int(az) == snap_azimuth_deg(self._azimuth):
    ...
    def _apply_cached_or_wait(self) -> None:
        az = snap_azimuth_deg(self._azimuth)
    ...
    def _set_precache_status(self, *, ready: bool = False) -> None:
        n = len(azimuth_cache_bins())
        have = len(self._atlas)
        az = snap_azimuth_deg(self._azimuth)
        if ready or have >= n:
            self._set_status(
                f"Pre-cache ready · az {az}° elev {self._elevation:.0f}° "
                f"Z×{self._z_factor:g} · analysis = height"
            )
            return
        self._set_status(f"Caching {have}/{n} · az {az}°…")
