...
class UI_MainWindow(QWidget):
    ...
    def setup_option_dock(self) -> None:
        ...
        self.checkbox_load_normalize = QCheckBox("Normalize each image")
        self.checkbox_load_normalize.setToolTip(
            "On load: stretch each image/frame to full range (per-image). "
            "Works for 8/16 bit and float. Off = comparable brightness across images."
        )
        ...
        # --- Shade (hillshade / Schummerung; view-only) ---
        shade_layout = QVBoxLayout()
        shade_heading = QLabel("Shade")
        shade_heading.setStyleSheet(get_style("heading"))
        shade_layout.addWidget(shade_heading)
        ...
        self.spinbox_shade_azimuth.setDecimals(0)
        ...
        self.spinbox_shade_elevation.setRange(0.0, 90.0)
        self.spinbox_shade_elevation.setDecimals(0)
        ...
        self.checkbox_shade_precached.setToolTip(
            "Freeze elevation and Z factor, then pre-render hillshade at the "
            "chosen azimuth step in the background. Azimuth scrub then swaps "
            "cached frames instead of recomputing. Paint optimization only — "
            "analysis still uses height."
        )
        shade_precached_layout.addWidget(self.checkbox_shade_precached)
        self.spinbox_shade_cache_step = QSpinBox()
        self.spinbox_shade_cache_step.setPrefix("Step: ")
        self.spinbox_shade_cache_step.setSuffix("°")
        self.spinbox_shade_cache_step.setRange(5, 90)
        self.spinbox_shade_cache_step.setSingleStep(5)
        self.spinbox_shade_cache_step.setValue(30)
        shade_precached_layout.addWidget(self.spinbox_shade_cache_step)
        self.label_shade_cache_ram = QLabel("12 frames at 30° · 5° finest")
        self.label_shade_cache_ram.setWordWrap(True)
        self.label_shade_cache_ram.setStyleSheet("color: #888; font-size: 10pt;")
        shade_precached_layout.addWidget(self.label_shade_cache_ram)
