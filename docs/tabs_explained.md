...
Frame range / step (video), resize, 8-bit, grayscale, normalize, pre-load ROI + Flip X/Y/XY, mixed-size policy. File-tab load options apply to File / Folder `.npy` and to Network ingest (same contract).
